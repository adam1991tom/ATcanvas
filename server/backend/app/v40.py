...
def _google_error(exc):
    if isinstance(exc, urllib.error.HTTPError):
        try:
            ...
            msg=data.get('error_description') or data.get('error',{}).get('message') or data.get('error') or body
            return str(msg)
        except Exception:
            return f'Google HTTP {exc.code}'
    return str(exc)


def _json_request(url, method='GET', data=None, token=None):
    headers={'User-Agent':'AT-Canvas/0.4','Accept':'application/json'}
    body=None
    ...
    try:
        ...
        raise HTTPException(502,_google_error(exc))


def _access_token():
    s=_settings(['google_client_id','google_client_secret','google_access_token','google_refresh_token','google_token_expires'])
    ...
    _set({'google_access_token':token,'google_token_expires':int(time.time())+int(data.get('expires_in',3600))})
    return token
...
def google_status(request: Request):
    s=_settings(['google_client_id','google_client_secret','google_refresh_token','google_access_token','google_redirect_uri'])
    ...
    return {
        ...
        'connected':bool(s.get('google_refresh_token') or s.get('google_access_token')),
        'client_id':s.get('google_client_id',''),
        'has_secret':bool(s.get('google_client_secret')),
...
def google_auth_start(request: Request):
    ...
    _set({'google_oauth_state':state,'google_redirect_uri':redirect})
    ...
    return {'url':AUTH_URL+'?'+q,'redirect_uri':redirect}


@app.get('/api/google/oauth/callback', response_class=HTMLResponse)
def google_oauth_callback(code: str='', state: str='', error: str=''):
    if error:
        return f'''<!doctype html><html><body style="font-family:system-ui;background:#0b0710;color:#fff;padding:40px"><h2>Google connection failed</h2><p>{error}</p></body></html>'''
    ...
    data=_json_request(TOKEN_URL,'POST',{
        ...
        'redirect_uri':s.get('google_redirect_uri',''),
    ...
    _set(values); _delete(['google_oauth_state'])
...
def google_disconnect():
    _delete(['google_access_token','google_refresh_token','google_token_expires','google_selected_calendars','google_oauth_state'])
    return {'disconnected':True}


@app.get('/api/google/calendars')
def google_calendars():
    token=_access_token(); selected=set(_selected_ids())
    data=_json_request(CALENDAR_API+'/users/me/calendarList?'+urllib.parse.urlencode({'maxResults':250}),token=token)
    out=[]
    for c in data.get('items',[]):
        out.append({'id':c.get('id'),'summary':c.get('summaryOverride') or c.get('summary') or c.get('id'),'primary':bool(c.get('primary')),'selected':c.get('id') in selected,'backgroundColor':c.get('backgroundColor'),'accessRole':c.get('accessRole')})
    return out
...
def google_calendar_selection(body: CalendarSelection):
    ...
    _set({'google_selected_calendars':json.dumps(cleaned)})
...
def google_events(days: int=14, limit: int=20):
    ...
    events=[]
    for cid in ids:
        q=urllib.parse.urlencode({'timeMin':now.isoformat().replace('+00:00','Z'),'timeMax':end.isoformat().replace('+00:00','Z'),'singleEvents':'true','orderBy':'startTime','maxResults':limit})
        url=CALENDAR_API+'/calendars/'+urllib.parse.quote(cid,safe='')+'/events?'+q
        data=_json_request(url,token=token)
        for e in data.get('items',[]):
            start=e.get('start',{}); finish=e.get('end',{})
            events.append({'id':e.get('id'),'summary':e.get('summary') or '(No title)','start':start.get('dateTime') or start.get('date'),'end':finish.get('dateTime') or finish.get('date'),'all_day':'date' in start,'location':e.get('location',''),'calendar_id':cid,'calendar_name':data.get('summary') or cid})
    events.sort(key=lambda e:e.get('start') or '')
    return {'events':events[:limit],'selected_calendars':len(ids)}


# Turn the existing Calendar placeholder into a live server-backed widget everywhere the renderer is used.
...
def _google_render(layout,layers):
    ...
    widget='''<div class="at-gcal" style="width:100%;height:100%;padding:18px;overflow:hidden;text-align:left"><div style="font-weight:800;margin-bottom:10px">Upcoming</div><div class="at-gcal-events" style="display:grid;gap:8px;font-size:.62em">Loading calendar…</div></div>'''
    page=page.replace('Calendar widget',widget)
    script='''<script>(async()=>{try{const r=await fetch('/api/google/events?days=14&limit=8');const j=await r.json();document.querySelectorAll('.at-gcal-events').forEach(root=>{if(!r.ok){root.textContent=j.detail||'Google Calendar error';return}if(!j.events.length){root.textContent='No upcoming events';return}root.innerHTML=j.events.map(e=>{const d=e.all_day?new Date(e.start+'T00:00:00'):new Date(e.start);const when=e.all_day?d.toLocaleDateString('en-GB',{weekday:'short',day:'numeric',month:'short'}):d.toLocaleString('en-GB',{weekday:'short',day:'numeric',month:'short',hour:'2-digit',minute:'2-digit'});return `<div style="padding:8px 0;border-bottom:1px solid rgba(255,255,255,.15)"><div style="font-weight:700">${String(e.summary||'').replace(/[&<>]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[c]))}</div><div style="opacity:.7;font-size:.8em">${when} · ${String(e.calendar_name||'')}</div></div>`}).join('')})}catch(e){document.querySelectorAll('.at-gcal-events').forEach(x=>x.textContent='Calendar unavailable')}})();</script>'''
    return page.replace('</body>',script+'</body>')

v34.v33.v32.v31.v30.render_layout_html=_google_render


# Replace only the Calendar admin page and JS route; keep the rest of the proven v0.3.5 interface.
