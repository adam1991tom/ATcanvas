...
# Remove all dedicated-client/pairing routes and replace the display model with browser URLs.
_REMOVE = {
    ('/api/pair/request', 'POST'),
    ('/api/pair/claim', 'POST'),
    ('/api/pair/status/{code}', 'GET'),
...
app.router.routes[:] = [r for r in app.router.routes if not any(
    getattr(r, 'path', None) == p and m in getattr(r, 'methods', set()) for p, m in _REMOVE
)]
# Dedicated client commands are no longer meaningful for browser-only endpoints.
app.router.routes[:] = [r for r in app.router.routes if not (
    getattr(r, 'path', None) == '/api/displays/{display_id}/command/{command}' and 'POST' in getattr(r, 'methods', set())
)]
...
def cleanup_browser_only_model():
    with DB() as c:
        # Pairing codes belong to the retired client model. Existing display rows are preserved
        # because their tokens are now useful permanent browser endpoint IDs.
...
def create_display_endpoint(body: DisplayEndpointCreate, request: Request):
    ...
    with DB() as c:
        ...
        if body.schedule_id and not c.execute('SELECT 1 FROM schedules WHERE id=?', (body.schedule_id,)).fetchone():
        ...
        cur = c.execute('''INSERT INTO displays(name,room,token,resolution,orientation,brightness,current_layout,client_version,last_seen,desired_command,layout_id,test_mode,schedule_id)
                           VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)''',
                        (name, None, token, 'browser', 'landscape', 100,
                         layout['name'] if layout else 'Unassigned', 'browser-url', 0, None,
                         body.layout_id, 0 if body.layout_id else 1, body.schedule_id))
        did = cur.lastrowid
...
def delete_display_endpoint(display_id: int):
    with DB() as c:
        ...
        if cur.rowcount == 0:
            raise HTTPException(404, 'Display endpoint not found')
    return {'deleted': True}
...
def browser_displays(request: Request):
    with DB() as c:
        rows = c.execute('''SELECT d.*, s.name schedule_name, l.name layout_name
                            FROM displays d
        ...
        for row in rows:
            d = dict(row)
            d['online'] = True
            d['endpoint_ready'] = True
            d['display_url'] = f'{_base(request)}/display/{row["token"]}'
            d['active_schedule'] = v33.v32.v31.v30._active_block(c, row['schedule_id'])
            out.append(d)
        return out
...
def browser_display(token: str):
    with DB() as c:
        ...
        block = v33.v32.v31.v30._active_block(c, d['schedule_id'])
        action = block['action'] if block else None
        if action == 'screen_off':
            return _black_page(d['name'])
        ...
        if d['test_mode'] or not layout_id:
            return _reload(v33.v32.v31.v30.TEST_HTML.replace('DISPLAY TEST', html.escape(d['name']) + ' · READY'))

        layout = c.execute('SELECT * FROM layouts WHERE id=?', (layout_id,)).fetchone()
        if not layout:
            return _reload(v33.v32.v31.v30.TEST_HTML.replace('DISPLAY TEST', 'LAYOUT NOT FOUND'))
        layers = c.execute('SELECT * FROM layers WHERE layout_id=? ORDER BY z', (layout_id,)).fetchall()
        page = v33.v32.v31.v30.render_layout_html(layout, layers)
        if action == 'dim':
            try:
...
def browser_update_status():
    ...
    try:
        ...
        req = urllib.request.Request('https://api.github.com/repos/adam1991tom/ATcanvas/commits/server', headers=headers)
...
def admin_v34():
    ...
    src = src.replace('Monitor and remotely control every paired screen.', 'Create and manage permanent browser display URLs.')
    src = src.replace('Server and display update status.', 'AT Canvas server release status.')
    return src


@app.get('/admin-v2.js')
def admin_v34_js():
    base = BASE.UI_FILE.with_name('admin_v33.js').read_text()
    patch = BASE.UI_FILE.with_name('admin_v34_patch.js').read_text()
