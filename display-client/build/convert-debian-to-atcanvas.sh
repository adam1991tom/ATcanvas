...
if [[ -f /etc/default/grub ]]; then
  ...
  if grep -q '^GRUB_CMDLINE_LINUX_DEFAULT=' /etc/default/grub; then
    sed -i 's/^GRUB_CMDLINE_LINUX_DEFAULT=.*/GRUB_CMDLINE_LINUX_DEFAULT="quiet splash loglevel=3 systemd.show_status=false vt.global_cursor_default=0 rd.systemd.show_status=false"/' /etc/default/grub
  fi
  update-grub || true
fi
...
# Install the branded Plymouth boot screen from the repo.
chmod +x "$INSTALL_DIR/display-client/branding/install-branding.sh"
AT_CANVAS_ROOT_DIR="$INSTALL_DIR" "$INSTALL_DIR/display-client/branding/install-branding.sh"
...
echo "AT Canvas branded boot splash installed."
